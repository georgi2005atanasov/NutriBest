...
export default function ProductsList({ productsRows }) {
    return productsRows && productsRows.length && productsRows.length != 0 && productsRows.map(row => <div key={row[0].name} className="row d-flex justify-content-between mb-4">
        {row.map(p => {
